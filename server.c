...
int reply_testing(struct socket *sock)
{
	struct msghdr msg;
	struct kvec vec;
	int written = 0;
...
void  tcp_server_thread(void) {
	...
	while(!kthread_should_stop() ) {
		...
		len = RcvBuf(test_socket, buffer, 1024);
		printk(KERN_INFO "Received: %s\n", buffer);

		send_rv = reply_testing(test_socket);	
...
static ssize_t mwrite(struct file *file, const char __user *ubuf, size_t count, loff_t *offset)
{
	...
	if( (my_strcmp(pin, run_command) == -1) && (serv_sock_alive == 0)) {
		...
		// creating a socket in server module.
		...
		base->thread = kthread_run( (void *) tcp_server_thread, NULL, MODULE_NAME);
...
static int module_init0(void)
{
	...
	printk(KERN_INFO "--Module loaded into kernel.\n");
